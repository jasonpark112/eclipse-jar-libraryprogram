...
public class Application {
    ...
    public static void numberOne(BufferedReader br) throws IOException {
        ...
        for (int i = 0; i < members.size(); i++) {
            if (name.equals(members.get(i).name.strip())) {
                bookName = members.get(i).bookName.strip();
                expectedRD = LocalDate.parse(members.get(i).expectedRD.strip(), DATE_PATTERN);
                realRD = LocalDate.parse(members.get(i).realRD.strip(), DATE_PATTERN);

                for (int j = 0; j < books.size(); j++) {
                    if (bookName.equals(books.get(j).bookName.strip())) {
                        category = books.get(j).category.strip();
                    }
                }
    ...
    public static void numberTwo(BufferedReader br) throws IOException{
        ...
        for (int i = 0; i < books.size(); i++) {
            ...
            if (category.equals(books.get(i).category.strip())) {
                count++;
